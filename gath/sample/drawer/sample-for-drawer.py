...
def draw_with_ckpt():
    d = GathDrawer.from_ckpt(env.anything_v5_ckpt)
...
def draw_with_model_and_vae():
    vae = AutoencoderKL.from_pretrained('E:\\OneDrive\\Leqee\\ai\\civitai\\vae_yozora_diffuser')
    d = GathDrawer.from_pretrained(env.sd_v1_5_model, vae=vae)
    ...
    # 提示词
    prompt = """
        a LeXiaoQi, walking on the water, wearing a dress, spring, sakura blossoms, looking at viewer,
        """
    # 降噪步数 数字越大，时间越长
    num_inference_steps = 30
    # 遵循提示词的级别 数字越大越接近提示词，但图像质量会下降
